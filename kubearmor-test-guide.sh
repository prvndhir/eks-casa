...
# Get the pod name
export POD=$(kubectl get pod -l app=nginx -o name -n nginx)

# Try to run apt / apt-get
kubectl exec -it $POD -n nginx -- bash -c "apt update && apt install masscan"
...
kubectl annotate ns nginx kubearmor-visibility="process, file, network" --overwrite

# Apply the policy to audit access to folders/paths
kubectl apply -f ./audit-etc-nginx-access.yaml

# Try to access /etc/nginx
kubectl exec -it $POD -n nginx -- bash -c "head /etc/nginx/nginx.conf"
...
# By default the security posture is set to audit. Lets change the security posture to default deny.
kubectl annotate ns nginx kubearmor-file-posture=block --overwrite

# Try to run a few commands
kubectl exec -it $POD -n nginx -- bash -c "passwd"
kubectl exec -it $POD -n nginx -- bash -c "ls"
kubectl exec -it $POD -n nginx -- bash -c "chroot"

# Verify policy violations
karmor logs -n nginx

# Verify you can still access nginx web page
kubectl port-forward $POD -n nginx --address 0.0.0.0 8080:80
curl localhost:8080
